...
class MianReactor implements Runnable {
    ...
    final int subReactorSize = 3;
    SubReactor[] subReactors = new SubReactor[3];
    ThreadPoolExecutor reactorPool = createThreadPool();
    ...
    private class SubReactor implements Runnable {
        ...
        public void run() {
            try {
                ...
                if (accept != null) {
                    new Handler(selector, accept, threadPoolExecutor);
                }
            } catch (IOException e) {
                e.printStackTrace();
            }
...
//                }
            ...
            reactorPool.execute(subReactors[index]);
